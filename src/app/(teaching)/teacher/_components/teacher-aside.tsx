...
import { BookOpen, GraduationCap, HelpCircle, LayoutGrid, LogOut, X } from "lucide-react";
...
import type { TeacherNavTab } from "../_utils/teacher-nav";
...
const NAV_ITEMS: {
  ...
  { id: "sessions", icon: BookOpen, href: "/teacher/sessions" },
  { id: "questions", icon: HelpCircle, href: "/teacher/courses/questions" },
];

function getActiveTab(pathname: string): TeacherNavTab {
  if (pathname.startsWith("/teacher/courses/questions")) return "questions";
  return "sessions";
}

type TeacherAsideProps = {
  mobileOpen: boolean;
  onClose: () => void;
};

export function TeacherAside({ mobileOpen, onClose }: TeacherAsideProps) {
  const translations = useTranslations();
  const t = translations.teacher.sidebar;
  ...
  const displayName = getUserDisplayName(firstname, lastname, t.teacherRole);
    ...
    >
      ...
      <div className="flex items-center gap-2.5 border-b border-border px-4 py-3 pb-4">
        <div className="flex size-8 shrink-0 items-center justify-center rounded-[9px] bg-primary">
          <GraduationCap className="size-[17px] text-primary-foreground" />
        </div>
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-bold leading-tight text-foreground">{displayName}</p>
          <p className="text-[11px] text-landing-subtle">{t.teacherRole}</p>
        </div>
        <button
          type="button"
          ...
          data-testid="button-close-teacher-nav"
        >
