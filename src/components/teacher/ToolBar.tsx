...
export function ToolBar({ setSelectedSession, handleOpenCreateModal, sessions }: ToolBarProps) {
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      ...
      const filtered = sessions.filter((session) => session.name.toLowerCase().includes(query));
            ...
            {searchQuery && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-background border rounded-lg shadow-lg z-10 max-h-80 overflow-y-auto">
                {isSearching ? (
                  ...
                  <div className="p-4 text-center text-muted-foreground">{t.teacher.noClassFound}</div>
                ) : (
                  <div className="py-2">
                    {searchResults.map((session) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-muted-foreground truncate">Code: {session.code}</p>
                        </div>
