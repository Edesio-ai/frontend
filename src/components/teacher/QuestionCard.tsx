...
import { correctAnswerDisplay, indexMatchesCorrectAnswer, propositionLabels } from "@/lib/proposition-labels";
...
export function QuestionCard({
  ...
  const [editedProposals, setEditedPropositions] = useState<string[]>(
    initialLabels.length > 0 ? initialLabels : ["Option A", "Option B", "Option C", "Option D"],
  );
  ...
  const [editedExplanation, setEditedExplication] = useState(question.explanation || "");
  ...
  const handleCancel = () => {
    ...
    setEditedPropositions(labels.length > 0 ? labels : ["Option A", "Option B", "Option C", "Option D"]);
    const resetIdx = labels.findIndex((_, i) =>
      indexMatchesCorrectAnswer(i, question.proposals, question.correctAnswers?.[0] || null),
    );
    setCorrectAnswerIndex(resetIdx >= 0 ? resetIdx : 0);
    setEditedGoodAnswer(correctAnswerDisplay(question.proposals, question.correctAnswers) || "");
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0 space-y-1">
          <div className="flex items-center justify-between gap-2">
            ...
            >
              {question.type === "single" || question.type === "multiple"
                ? t.teacher.questionCard.mcq
                : t.teacher.questionCard.open}
            </span>
            <Button
              size="sm"
              variant="ghost"
          ...
          {(question.type === "single" || question.type === "multiple") &&
            propositionLabels(question.proposals).length > 0 && (
