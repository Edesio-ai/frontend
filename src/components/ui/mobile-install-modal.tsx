    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md" data-testid="modal-mobile-install">
        <DialogHeader>
          ...
          <DialogDescription>{t.mobileInstall.title}</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-muted/50 rounded-lg p-3 text-sm text-muted-foreground">
            <p className="flex items-start gap-2">
              ...
              <span>The app works like a native app: quick access, full screen, and soon push notifications!</span>
            </p>
          </div>
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <p className="text-sm truncate">{t.mobileInstall.title}</p>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <Button size="sm" variant="default" onClick={onOpenModal} data-testid="button-show-install-instructions">
            <Download className="h-4 w-4 mr-1" />
            Install
          </Button>
          <Button size="sm" variant="ghost" onClick={handleDismiss} data-testid="button-dismiss-install-banner">
            Later
          </Button>
        </div>
      </div>
