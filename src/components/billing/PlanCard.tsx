    ...
    >
      {recommendedPlan === plan.id && (
        <div className="absolute top-0 right-0">
          <div
            className="bg-gradient-to-r from-primary to-primary/80 text-white text-xs font-bold px-3 py-1 rounded-bl-lg"
            data-testid={`badge-recommended-${plan.id}`}
          >
      ...
      <div className="relative z-10 flex flex-col h-full">
        <div className="mb-5">
          ...
          <p className="text-sm text-muted-foreground mb-4">{planDetails?.description ?? plan.description}</p>
        ...
        <ul className="space-y-2.5 mb-6 flex-1">
          {plan.features.map((feature, featureIndex) => (
            <li key={featureIndex} className="flex items-start gap-2.5">
              <div
                className={`w-4 h-4 rounded-full ${feature.highlight ? plan.iconBg : "bg-emerald-500/20"} flex items-center justify-center shrink-0 mt-0.5`}
              >
                <Check
                  className={`h-2.5 w-2.5 ${feature.highlight ? "text-white" : "text-emerald-600 dark:text-emerald-400"}`}
                />
              </div>
              <span className="text-sm">{planDetails?.features?.[featureIndex] ?? feature.text}</span>
            </li>
          ))}
        </ul>
