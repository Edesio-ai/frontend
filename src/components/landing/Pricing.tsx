    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-12 md:mb-16">
          ...
          >
            {pt.title}{" "}
            <span className="bg-gradient-to-r from-sky-400 to-purple-400 bg-clip-text text-transparent">{pt.titleHighlight}</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {pt.plans.map((plan: { name: string; description: string; target: string; cta: string; features: string[] }, index: number) => {
            const meta = planMeta[index];
            const Icon = planIcons[index];
            return (
              <Card
                key={meta.id}
                className={`relative p-6 flex flex-col overflow-hidden transition-all duration-300 border-none ${meta.popular
              ...
              >
                ...
                <div className="relative z-10 flex flex-col h-full">
                  <div className="mb-5">
                    <div className={`w-12 h-12 rounded-xl ${meta.iconBg} flex items-center justify-center shadow-lg mb-4`}>
                    ...
                    <div className="flex items-baseline gap-1">
                      {meta.monthlyPrice !== null ? (
                        ...
                        <span className={`text-2xl font-bold ${meta.accentColor}`}>
                          {pt.onQuote}
                        </span>
                      )}
                    </div>
                  ...
                  <ul className="space-y-2.5 mb-6 flex-1">
                    {plan.features.map((feature, featureIndex) => (
                      <li key={featureIndex} className="flex items-start gap-2.5">
                        <div className={`w-4 h-4 rounded-full ${featureIndex === 0 ? meta.iconBg : "bg-emerald-500/20"} flex items-center justify-center shrink-0 mt-0.5`}>
                          <Check className={`h-2.5 w-2.5 ${featureIndex === 0 ? "text-white" : "text-emerald-600 dark:text-emerald-400"}`} />
                        </div>
