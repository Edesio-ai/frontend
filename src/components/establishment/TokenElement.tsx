  ...
  return (
    <div
      ...
      className={`flex items-center justify-between p-4 rounded-lg border ${
        expired || used ? "bg-muted/50 opacity-60" : "bg-background"
      }`}
      data-testid={`invitation-token-${token.id}`}
    >
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-4">
          <code className="bg-muted px-3 py-1 rounded font-mono text-sm">{token.token}</code>
          {used ? (
            <Badge variant="secondary" className="flex items-center gap-1">
              <CheckCircle2 className="h-3 w-3" />
              {t.establishment.token.used}
            </Badge>
          ) : expired ? (
            <Badge variant="destructive" className="flex items-center gap-1">
              <Clock className="h-3 w-3" />
              {t.establishment.token.expired}
            </Badge>
          ) : (
