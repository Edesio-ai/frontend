...
export function SessionCoursesList({
  ...
  if (courses.length === 0) {
    return <p className="text-xs text-muted-foreground py-1">{t.establishment.sessionCoursesList.empty}</p>;
  }
          ...
          <div className="flex items-center gap-2">
            {cours.validatedQuestions ? (
              <Badge variant="default" className="text-xs">
                {t.establishment.sessionCoursesList.published}
              </Badge>
            ) : (
              <Badge variant="outline" className="text-xs">
                {t.establishment.sessionCoursesList.draft}
              </Badge>
            )}
